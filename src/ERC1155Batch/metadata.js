	...
	try {
		...
		if (transferData.from === NULL_ADDRESS) {
			...
			let tokenURI = await contract.methods
				.uri(tokenTransferDetails.ids[0])
				.call();
			...
			if (imageUrl) {
				...
				const path = `${transferData.contract_address}/${tokenTransferDetails.ids[0]}/`;
		...
		console.log({ ERC1155BATCH_METADATA: e.message });
		return { log, transferData };
	}
};
